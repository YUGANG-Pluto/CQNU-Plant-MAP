...
export const architectureLayers = Object.freeze([
  ...
  },
  {
    title: '浏览器能力适配器',
    state: '本地优先 + 可选云项目',
    detail: '浏览器默认使用授权目录、OPFS 与 SQLite WASM；用户也可主动把 settings、zones、points 记录保存到本人云项目。'
  },
  {
    title: '发布与文档站',
    state: '受限访问',
    detail: '当前站点仅限受控账户访问，提供文档、完整浏览器工作区、账户服务和按账户隔离的云项目快照。'
  }
]);
...
export const docsSections = Object.freeze([
  ...
  },
  {
    ...
    items: [
      ...
      '云项目库仅在用户主动创建或上传时接收 settings、zones、points 记录快照，其中可能包含点位坐标和相对图片引用；上传前会清除服务凭据、鉴权 URL 参数和设备绝对路径，不接收 SQLite/JSON 原文件、图片字节、备份、日志或目录句柄。',
      '云项目按账户隔离并使用递增版本、大小上限与 SHA-256 完整性校验；从云端打开后先建立 OPFS 工作副本，不会持续后台同步。',
      '清除浏览器站点数据会删除未另行导出的 OPFS 工作副本，但不会自动删除已显式保存的云项目版本。',
  ...
  },
  {
    ...
    items: [
      ...
      '缺少 OPFS、Web Locks 或其他关键能力时，工作区会禁用本地写入并建议使用当前 Chromium 浏览器或桌面版；不会静默上传项目，也不会把云项目当作本地能力回退。',
  ...
  },
  {
    ...
    items: [
      ...
      '只读权限可打开本地或本人云项目、浏览、查询、统计和导出；编辑权限增加会话草稿但不持久化；保存权限增加本地持久化、云项目创建与上传、图片、备份、恢复和存储转换。',
      '管理员最多 3 名，可管理其他成员、权限、激活链接、一次性密码重置链接和安全审计；管理员不能通过成员表修改自己的权限。',
      '账户服务保存认证、权限和安全审计；云项目服务只在显式操作时保存本人已清除凭据与设备路径的 settings、zones、points 记录快照，不接收图片字节、目录句柄或桌面数据库文件。',
