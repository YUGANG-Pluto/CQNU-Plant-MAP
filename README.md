...
- 浏览器工作区：完整界面位于 `/workspace`，由用户主动授权项目目录或选择文件；可本地导入桌面端 `.db`、`.sqlite`、`.sqlite3` 数据库，原文件保持只读，编辑结果保存在浏览器 OPFS SQLite 工作副本中。站点分支还提供按账户隔离的云项目库，仅在用户主动操作时上传已清除服务凭据与设备绝对路径的 `settings`、`zones`、`points` 记录快照。
...
- Browser workspace: the complete interface is available at `/workspace`. Users explicitly authorize a directory or select files. Desktop `.db`, `.sqlite`, and `.sqlite3` databases are immutable import sources, while edits use an OPFS SQLite working copy. The site branch also provides account-scoped cloud projects; only an explicit action uploads a `settings`, `zones`, and `points` snapshot after service credentials and device-absolute paths are removed.
