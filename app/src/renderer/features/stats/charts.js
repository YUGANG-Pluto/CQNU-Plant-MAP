...
function renderBarList(items, valueKey, labelBuilder, chartKey = 'barList') {
  ...
  rows.forEach((row, index) => {
    ...
    bars += `<g class="chart-bar-group" style="--chart-index:${index};--chart-delay:${Math.min(index * 56, 392)}ms;"><rect class="chart-bar-depth" x="${(layout.barX + 4).toFixed(1)}" y="${(y + 7).toFixed(1)}" width="${layout.barWidth.toFixed(1)}" height="${Math.max(0, barHeight - 2).toFixed(1)}" rx="10"></rect><rect class="chart-bar-rect" data-center-x="${layout.centerX.toFixed(1)}" data-band-width="${layout.bandWidth.toFixed(1)}" x="${layout.barX.toFixed(1)}" y="${y.toFixed(1)}" width="${layout.barWidth.toFixed(1)}" height="${barHeight.toFixed(1)}" rx="10" fill="url(#barGrad${index % palette.length})">${svgTitle(chartMetricLabel(valueKey, valueKey), row.label, row.value)}</rect><rect class="chart-bar-highlight" data-center-x="${layout.centerX.toFixed(1)}" data-band-width="${layout.bandWidth.toFixed(1)}" x="${layout.barX.toFixed(1)}" y="${y.toFixed(1)}" width="${layout.barWidth.toFixed(1)}" height="${Math.min(14, Math.max(0, barHeight)).toFixed(1)}" rx="10" fill="url(#barGloss)"></rect>${label}</g>`;
...
function renderComboChart(rows, barMetric = 'count', lineMetric = 'percentage', chartKey = 'combo') {
  ...
  const categoryGroups = categories.map(item => {
    ...
    return `<g class="chart-combo-category" data-category-index="${item.index}" style="--chart-index:${item.index};--chart-delay:${Math.min(item.index * 56, 392)}ms;">
...
function donutSvgFromCounts(entries, palette, settings, donut = true, chartKey = 'donut') {
  ...
  let offset = 0;
  const gradients = safeEntries.map((_, index) => {
  ...
  const slices = safeEntries.map(([label, value], index) => {
    ...
    const slice = fraction >= 0.999 && !donut
      ? `<circle class="donut-slice donut-slice-full" data-slice-index="${index}" style="--slice-index:${index};--slice-delay:${Math.min(index * 64, 448)}ms;" cx="${center}" cy="${center}" r="${outerRadius.toFixed(2)}" fill="url(#${idPrefix}Grad${index})"><title>${escapeHtml(label)}：${escapeHtml(value)} / ${percent}</title></circle>`
      : `<path class="donut-slice" data-slice-index="${index}" style="--slice-index:${index};--slice-delay:${Math.min(index * 64, 448)}ms;" d="${path}" fill="url(#${idPrefix}Grad${index})" fill-rule="evenodd"><title>${escapeHtml(label)}：${escapeHtml(value)} / ${percent}</title></path>`;
    offset += fraction;
    return slice;
  }).join('');
...
function renderMiniLegend(entries, palette) {
  return entries.map(([label, value], index) => `<div class="legend-item" style="--legend-index:${index};--legend-delay:${Math.min(index * 48, 336)}ms;"><span class="legend-dot" style="background:${palette[index % palette.length]}"></span><span>${escapeHtml(label)}</span><strong>${value}</strong></div>`).join('');
}
