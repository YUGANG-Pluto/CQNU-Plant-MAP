function testHtmlErrorDialogWiring() {
  ...
  assert.ok(loaderSource.indexOf('./src/renderer/utils/dialogs.js') < loaderSource.indexOf('./src/renderer/utils/errorHandler.js'));
}
...
function testModernMotionContract() {
  ...
  [
    ...
    '@keyframes uiPanelIn'
  ].forEach(keyframe => assert.ok(globalSource.includes(keyframe), `${keyframe} must stay available`));
