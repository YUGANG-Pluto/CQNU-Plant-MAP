...
function testEngineeringSplitContract() {
  ...
  const themeModelPath = path.join(modernRoot, 'features/theme/model.ts');
  const themeRuntimePath = path.join(modernRoot, 'features/theme/runtime.ts');
  const themeModel = fs.readFileSync(themeModelPath, 'utf8');
  const themeRuntime = fs.readFileSync(themeRuntimePath, 'utf8');
  const themeContracts = fs.readFileSync(path.join(modernRoot, 'features/theme/contracts.ts'), 'utf8');
  const themePresets = fs.readFileSync(path.join(modernRoot, 'features/theme/presets.ts'), 'utf8');
  const themeNormalization = fs.readFileSync(path.join(modernRoot, 'features/theme/normalization.ts'), 'utf8');
  const themeDocument = fs.readFileSync(path.join(modernRoot, 'features/theme/document.ts'), 'utf8');
  const themeController = fs.readFileSync(path.join(modernRoot, 'features/theme/controller.ts'), 'utf8');
  ...
  assert.ok(themeContracts.includes("'scientific-white' | 'liquid-glass'"));
  assert.ok(themePresets.includes("DEFAULT_UI_STYLE_ID: ThemeStyleId = 'scientific-white'"));
  assert.ok(themeModel.includes("from './contracts'"));
  assert.ok(themeModel.includes("from './normalization'"));
  assert.ok(themeRuntime.includes('installLegacyThemeBridge'));
  assert.ok(themeRuntime.includes("from './controller'"));
  assert.ok(themeController.includes("guard('save-theme')"));
  assert.ok(themeNormalization.includes('normalizeThemeSettings'));
  assert.ok(themeDocument.includes('createThemeDocumentSnapshot'));
  assert.ok(themeDocument.includes('applyThemeToDocument'));
  assert.ok(themeModel.split(/\r?\n/).length <= 40, 'theme model must remain a small compatibility facade');
  assert.ok(themeRuntime.split(/\r?\n/).length <= 60, 'theme runtime must remain a small legacy bridge');
