...
Passwords use salted PBKDF2-HMAC-SHA-256 with a deployment-provided HMAC pepper. The shared implementation defaults to 600,000 iterations; the Sites Worker uses the Cloudflare runtime ceiling of 100,000 iterations. Every verifier stores its iteration count so the cost can be raised transparently when the platform permits it. Temporary credentials require first-use activation, while user-selected passwords remain subject to the weak-password blocklist and the six-character minimum. A user may defer the personal-password change after activation; the account remains visibly marked until a policy-compliant password is saved. Purpose-separated HMAC keys sign or digest session, CSRF, activation, and password-reset material. Production cookies are host-only, `HttpOnly`, `Secure`, and `SameSite=Strict`; state-changing account requests also require a same-origin CSRF token.
...
The administrator-only bulk identity reset requires same-origin CSRF validation, the current administrator password, and a fixed confirmation phrase. It keeps account IDs, usernames, roles, permissions, and cloud project ownership unchanged, but sets every account to pending activation with the known temporary password `123456`, invalidates outstanding activation and reset links, revokes all sessions including the initiator's, and records only account/session counts in the audit log. Because this is a shared known credential, the private hosting owner gate remains a required outer boundary until each account is reactivated and personalized.
