...
Decision record for explicit SQLite runtime work. The `better-sqlite3` dependency probe is active, and a guarded project storage conversion utility is available for explicit maintenance actions. JSON remains the compatibility format.
...
| Active project storage | JSON by default; SQLite after explicit conversion to `information/data.db`. |
| Renderer database access | Not allowed. |
| Main process database access | Allowed only through narrow storage and conversion services. |
...
| SQLite runtime acceptance | Pass: `db:test-runtime` verifies automatic SQLite priority, SQLite save, explicit JSON fallback, export back to JSON, and source cleanup. |
...
| Active SQLite runtime storage | Ready after explicit maintenance conversion. |
...
## Required Probe Before Release Use

Before release use of SQLite runtime storage, verify:
...
5. `npm run db:test-runtime` passes.
6. No SQLite binary, database file, or generated build output is committed.

## Runtime Boundary

SQLite access must stay behind a main-process service. Renderer code must not receive:
...
The renderer may request only business operations such as conversion preflight, creating SQLite storage, exporting back to JSON, artifact cleanup, backup restore, and report retrieval.
...
This decision authorizes the guarded project conversion utility, explicit SQLite runtime storage after conversion, dependency probes, and runtime acceptance checks. Making SQLite the default recommendation for all new projects still requires a release-level UI smoke test and rollback verification on a copied project.
